...
function CarouselProvider({
  ...
  const api = useMemo<CarouselApi>(
    () => ({
      canScrollPrev,
      canScrollNext,
      scrollPrev: () => handleSetIndex(Math.max(0, index - 1)),
      scrollNext: () =>
        handleSetIndex(Math.min(itemsCount - 1, index + 1)),
      on: (event, callback) => {
...
function Carousel({
  ...
  const [responsiveDisableDrag, setResponsiveDisableDrag] =
    useState<boolean>(disableDrag);
  ...
  useEffect(() => {
    ...
    const mediaQueries = Object.entries(opts.breakpoints).map(
      ([query, settings]) => {
        const mql = window.matchMedia(query);
        const handler = (ev: MediaQueryListEvent | MediaQueryList) => {
          // Enable drag when dragFree is true, otherwise respect disableDrag prop.
          const shouldDisable =
            disableDrag && !settings.dragFree ? true : false;
          setResponsiveDisableDrag(shouldDisable);
        };
        handler(mql);
  ...
  return (
    <CarouselProvider
      ...
      disableDrag={responsiveDisableDrag}
      setApi={setApi}
    >
...
function CarouselContent({
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      const visibleCount = entries.filter((entry) => entry.isIntersecting)
        .length;
